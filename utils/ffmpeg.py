...
def gen_ac3_audio(input_file, output_file, all_ac3_list, audio_index_list, shell, add_dolby=True, **kwargs):
    multi_lang = kwargs.get('multi_lang', False)
    need_get_ac3_list = kwargs.get('need_get_ac3_list', [])
    ret = True
    if add_dolby and (all_ac3_list or audio_index_list or multi_lang):
        transcode_options = ["-vn", "-acodec", "ac3", "-ab", "320k", "-ac", "6"]
        if multi_lang:
            ac3_list = need_get_ac3_list
        else:
            if all_ac3_list:
...
def process_video(input_file, output_file, media_info, offset, duration, target_lang, shell, copy_mode=True, gpu=True,
    ...
    try:
        ...
        ret = False
        logger.warning('transcode video failed %s params=[%s]', e, params)
...
def process_audio(input_file, output, media_info, shell, timeout=3600, add_dolby=True, **kwargs):
        ...
        for stream in media_info['stream']:
            if stream['codec_type'].startswith('audio'):
                ...
                if 'has_aac' not in language_state[stream['language']].keys() or \
                    ...
                    language_state[stream['language']]['has_aac'] = (True, stream['index']) if stream[
                                                                                                   'codec_name'].find(
                        'aac') >= 0 else (False, 0)
                ...
                if 'has_eac3' not in language_state[stream['language']].keys() or \
                    ...
                    language_state[stream['language']]['has_eac3'] = (True, stream['index']) if stream[
                                                                                                    'codec_name'].find(
                        'eac3') >= 0 else (False, 0)
    ...
    '''
    1. 当只有ac3音轨或者有其他音轨的时候转换成aac
    2. 存在多语言的时候, 每一种语言都需要有aac和ac3音轨
    '''
    audio_state_list = []
    for stream in media_info['stream']:
        if stream['codec_type'].startswith('audio'):
            ...
            if only_have_ac3:
                ...
                has_other_aduio = stream['codec_name'].find('aac') < 0 and stream['codec_name'].find('ac3') < 0
            ...
            try:
                audio_state_dic['channels'] = int(stream['channels']) if stream['channels'] and 'channels' in stream.keys() else 0
            except BaseException as e:
    ...
    for audio_state in audio_state_list:
        ...
        if audio_state["target_codec_name"].find('ac3') >= 0:
            if not channels:
                channels = SUPPORT_CHANNEL_NUMBER
                bit_rate = BITE_RATE_MAX
            else:
                if channels <= 2:
                    channels = 2
                    bit_rate = BITE_RATE_MEDIUM
                elif channels >= SUPPORT_CHANNEL_NUMBER:
                    channels = SUPPORT_CHANNEL_NUMBER
                    bit_rate = BITE_RATE_MAX
        ...
        audio_process_result = ffmpeg_audio_tracks_process(input_file, audio_tmp, audio_tracks_list, shell,
                                                           ...
                                                           target_codec=audio_state["target_codec_name"])
        if audio_process_result:
            # 检查转码完成的文件时长是否异常
            check_audio_ret = check_file_duration(audio_tmp, video_duration)
            # 多语言的情况下, 可能出现其中有一个语言的时长是正确的, 需要保留正确的那个
            if multi_lang:
                # 检测时长和视频不一致, 并且这个音轨需要生成ac3的话, 需要从生成ac3的列表中删除此音轨
                if not check_audio_ret and audio_state['audio_index'] in need_get_ac3_list:
                    need_get_ac3_list.pop(need_get_ac3_list.index(audio_state['audio_index']))
                    continue
            if not check_audio_ret:
                    audio_process_result = False
                    break
            audio_src_dic['src'] = audio_tmp
            audio_src_dic['target_codec_name'] = audio_state["target_codec_name"]
    ...
    if audio_process_result:
        ...
        gen_ac3_ret = False
        if not all_ac3_list or only_have_ac3 or multi_lang:
            gen_ac3_ret = gen_ac3_audio(input_file, output_ac3_tmp, all_ac3_list, audio_index_list, shell,
                                        add_dolby=add_dolby, multi_lang=multi_lang, need_get_ac3_list=need_get_ac3_list)
        if gen_ac3_ret:
            audio_src_list.append({'src': output_ac3_tmp, 'target_codec_name': 'ac3'})
...
def transcode(src, output, gpu=True, offset=0.0, duration=0.0, copy_mode=False, mp4=False,
    ...
    audio_process_result = False
    if video_ret:
        video_duration = int(float(ffprobe_get_media_duration(video_tmp)))
    ...
    if video_ret and audio_src_list and audio_process_result:
        ...
        if video_ret and audio_process_result:
            failed_reason = 'CombineError'
        elif video_ret:
            failed_reason = 'AudioError'
        else:
            failed_reason = 'VideoError'
    ...
    if os.path.exists(video_tmp):
        ...
        logger.warning(f'not exists file -> {video_tmp}')

    for audio_tmp in audio_src_list:
        if os.path.exists(audio_tmp):
            os.remove(audio_tmp)
        else:
            logger.warning(f'not exists file -> {audio_tmp}')
    return combine_ret, video_duration, failed_reason
